...
import { createAddress, deleteAddress } from "@/app/(dashboard)/clientes/actions";
...
export function AddressManager({
  ...
  } = useForm<z.input<typeof addressSchema>>({ resolver: zodResolver(addressSchema) });

  async function onSubmit(values: z.input<typeof addressSchema>) {
    const result = await createAddress(clientId, values as AddressInput);
      ...
      {adding ? (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 rounded-md border border-border p-3">
          <div>
            ...
            <Input id="label" placeholder="Ej: Edificio Alberdi 128" {...register("label")} />
          </div>
          <div>
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => { reset(); setAdding(false); }}>
              Cancelar
            </Button>
          </div>
        </form>
